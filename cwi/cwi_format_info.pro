		...
		if file_test(ifil,/read) then begin
			...
			while not eof(il) do begin
				...
				if strpos(rec,'UTC') ge 0 and not got_date then begin
					...
					jnk = gettok(rec,' ')
					ifi = file_info(ifil)
					year = fix((strsplit(systime(0,ifi.mtime),/extract))[4])
					dayno = fix(gettok(rec,' '))
					caldat,julday(1,dayno,year), mo, day
					timstr = strtrim(rec,2)
					val = string(year,mo,day,format='(i4,"-",i02,"-",i02)')+'T'+timstr
					ityp = 7
					after = 'EXPTIME'
					do_out = (1 eq 1)
					got_date = (1 eq 1)
