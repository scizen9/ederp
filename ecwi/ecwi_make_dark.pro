...
;	image numbers given in ECWI_PPAR struct PPAR.DARKS.
;
; CATEGORY:
...
;
; INPUTS:
;	Ppar	- ECWI_PPAR struct for dark group to combine
;
; KEYWORDS:
;	None
;
...
; EXAMPLE:
;
;	ppar = ECWI_PPAR_READ('mdark_0.ppar')
;	ECWI_MAKE_DARK,ppar
;
...
pro ecwi_make_dark,ppar
	...
	if strlen(ppar.darks) le 0 then begin
		ecwi_print_info,ppar,pre,'no darks entered',/error
		return
	endif
	;
	; directories
	...
	if nd ge ppar.mingroupdark then begin
		...
		ecwi_print_info,ppar,pre,'combining this many images',nd
	    	;
	    	; read first image
		...
		if file_test(dfil) then begin
			...
			ecwi_print_info,ppar,pre,'Image file not found', $
				dfil,/error
			return
		endelse
		; get size
		...
		if file_test(vfil) then begin
			...
			ecwi_print_info,ppar,pre,'Variance file not found', $
				vfil,/warning
			var = fltarr(sz)
		endelse
	    	;
		...
		if file_test(mfil) then begin
			...
			ecwi_print_info,ppar,pre,'Mask file not found', $
				mfil,/warning
			msk = intarr(sz)
		endelse
		;
		...
		for i=1,nd-1 do begin
			...
			if file_test(dfil) then begin
				...
				ecwi_print_info,ppar,pre,'File not found', $
					dfil,/error
				return
			endelse
			;
			...
			if sz[0] ne nx or sz[1] ne ny then begin
				ecwi_print_info,ppar,pre,'Size mismatch', $
					dfil,/error
				return
			endif
	    		;
		...
		ecwi_write_image,mdark,hdr,ppar.masterdark,ppar
		;
		...
		ecwi_write_image,mvar,varhdr,mastervar,ppar
		;
		...
		;
		; write out master variance
		ecwi_write_image,mmsk,mskhdr,mastermsk,ppar
	endif else $
		ecwi_print_info,ppar,pre,'too few dark frames',nd,/error
	;
	return
end
