...
;
; CALLING SEQUENCE:
;	ECWI_READ_LINKS, Ppar, LinkFilename, ImgNums
;
; INPUTS:
;	Ppar		- ECWI_PPAR pipeline parameter struct
;	LinkFilename	- link file (written with ECWI_PREP)
;
; OUTPUTS:
...
;-
pro ecwi_read_links,ppar,linkf,imgs, $
...
;
; check inputs
	if ecwi_verify_ppar(ppar,/silent) ne 0 then $
		ppar = { ecwi_ppar }
;
; initialize
	count = 0
...
; use defaults, if name not passed in
	if n_params(0) lt 1 or n_elements(linkf) le 0 then begin
		...
		if not file_test(linkf,/read) then begin
			...
			if not file_test(linkf,/read) then begin
				ecwi_print_info,ppar,pre,'default link file not found',/error
				return
			endif
		endif
	endif
...
; check file
	...
	if not fi.exists or not fi.read or not fi.regular then begin
		ecwi_print_info,ppar,pre,'file not accessible: ',linkf,/error
		return
	endif
;
; read file
