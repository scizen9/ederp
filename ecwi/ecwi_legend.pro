...
;-
pro ecwi_legend, items, BOTTOM_LEGEND=bottom, BOX = box, CENTER_LEGEND=center, $
...
;
on_error,2
if keyword_set(help) then begin & doc_library,'ecwi_legend' & return & endif
;
...
if n eq 0 then message,'No inputs!  For help, type ecwi_legend,/help.'
if ni eq 0 then begin
  items = replicate('',n)                               ; DEFAULT BLANK ARRAY
endif else begin
  if size(items,/TNAME) NE 'STRING' then message, $
      'First parameter must be a string array.  For help, type ecwi_legend,/help.'
...
case n_elements(position) of
 ...
 else: message,'Position keyword can have 0, 1, or 2 elements only. Try ecwi_legend,/help.'
endcase
