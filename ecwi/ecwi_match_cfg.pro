...
;
; PURPOSE:
;	This function returns the indices within Ecfg of images that
...
;
; CALLING SEQUENCE:
;	Result = ECWI_MATCH_CFG( KCFG, TCFG )
;
; INPUTS:
;	Ecfg	- array of struct ECWI_CFG for a given data set
;	Tcfg	- target struct ECWI_CFG to match
;	Ppar	- pipeline parameter ECWI_PPAR struct
;	Tlist	- list of ECWI_CFG struct tag names to match
;		   Defaults to ['XBINSIZE','YBINSIZE','AMPMODE',
;				'GRATID','GRATPOS','FILTER',
;				'FM4POS','CAMPOS','FOCPOS']
;
; Returns:
;	Ecfg entries that match the target configuration, -1 if none found.
;
; INPUT KEYWORDS:
;	SILENT	- set to suppress warning and error messages (for testing)
;	IMGTYPE	- set to ECWI image type to restrict match to: bias, dark, etc.
...
;	group contained in Ecfg and finds each entry that matches the
...
;-
function ecwi_match_cfg, ecfg, tcfg, ppar, tlist, $
	...
	if ecwi_verify_ppar(ppar,/init,/silent) ne 0 then begin
		ecwi_print_info,ppar,pre,'Ppar error - malformed ECWI_PPAR struct',/error
		return,-1
	endif
	if ecwi_verify_cfg(ecfg,/init,/silent) ne 0 then begin
		ecwi_print_info,ppar,pre,'Search Error - malformed ECWI_CFG struct array',/error
		return,-1
	endif
	if ecwi_verify_cfg(tcfg,/init,/silent) ne 0 then begin
		ecwi_print_info,ppar,pre,'Target Error - malformed ECWI_CFG struct array',/error
		return,-1
	endif
	;
	...
	for i=0,nmatch-1 do begin
		...
		if nt le 0 then begin
			ecwi_print_info,ppar,pre,'Unrecognized tag',mtags[i],/error
			return,-1
		endif
		tnum[i] = t[0]
	endfor
	...
	for i=0,ncfg-1 do begin
		...
		for j=0,nmatch-1 do begin
			...
			if ecfg[i].(tnum[j]) ne tcfg.(tnum[j]) then begin
				match = (1 eq 0)
			endif
		endfor
		;
		...
		if match then begin
			...
			if keyword_set(imgtype) then begin
				if strmatch(strtrim(ecfg[i].imgtype,2),imgtype) ne 1 then $
					mstat[i] = 0
			endif
			;
			...
			if strlen(tobj) gt 0 then begin
				...
				if strmatch(strtrim(ecfg[i].imgtype,2),'object') eq 1 then begin
					;
					; does the string appear?
					if strpos(ecfg[i].object,tobj) lt 0 then $
						mstat[i] = 0
		...
		;
		; time offset
		tdelt[i] = tcfg.juliandate - ecfg[i].juliandate
		;
		; sky coord offset
		if ecfg[i].ra ge 0. and ecfg[i].dec ge -90. then $
			gcirc,2,tcfg.ra,tcfg.dec,ecfg[i].ra,ecfg[i].dec,dis $
		else	dis = -99.
		cdelt[i] = dis
	endfor
	;
	...
	if count le 0 then begin 
		...
		if not keyword_set(silent) then $
			ecwi_print_info,ppar,pre,'No matches to image,type',tcfg.imgnum,tcfg.imgtype, $
				format='(a,i9,2x,a)',/error
	endif else mcfg = ecfg[mcfgi]
	;
	return,mcfg
end	; ECWI_MATCH_CFG
