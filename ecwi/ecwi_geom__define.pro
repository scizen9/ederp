...
;	ECWI geometric transformations.
;
; CATEGORY:
...
;
; SIDE EFFECTS:
;	defines the ECWI_GEOM data structure
;
; PROCEDURE:
;	Provide the automatic structure definition for the ECWI geometric
;	transformation structure ECWI_GEOM.
;
; EXAMPLE:
;	Instantiate and initialize a single geom struct for ECWI:
;
;	egeom = {ecwi_geom}
...
; cross correlation parameters
	ccwn:300., $		; cross correlation window for bar-to-bar offset
	ccoff:fltarr(12), $	; known offsets relative to reference slice in pixels
;
; wavelengths
...
; Wavelength fit for each bar
	bfitcoeffs:fltarr(9,60),$ ; polynomial wavelength solution
...
; Coefficients
	...
	ky:dblarr(4,4), $	;
	kwx:dblarr(6,6,12), $	; wavelength solution included for each slice
	kwy:dblarr(6,6,12), $	;
	ypad:600, $		; number of pixels to pad image in y
;
; Status
	xrsd:fltarr(12), $	; transform x residuals (pixels)
	yrsd:fltarr(12), $	; transform y residuals (pixels)
