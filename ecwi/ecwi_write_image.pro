...
;
; CALLING SEQUENCE:
;	ECWI_WRITE_IMAGE, Img, Hdr, FileName, Ppar
;
; INPUTS:
...
;		output path
;	Ppar	- ECWI_PPAR struct specifying outputs and logging flags
;
; KEYWORDS:
...
;	ECWI_PPAR struct Ppar.  Updates logfile if LOGLUN specified
...
;-
pro ecwi_write_image,img,hdr,filename,ppar,iscale=iscale,help=help
	;
	; setup
	pre = 'ECWI_WRITE_IMAGE'
	;
	; help request
	...
	;
	; check inputs
	odir = ecwi_expand_dir(ppar.reddir)
	...
	if file_test(ofil) then begin
		...
		if ppar.clobber eq 1 then begin
			...
			ecwi_print_info,ppar,pre,'deleted existing file',ofil,format='(a,a)'
		;
		; or skip writing and report
		endif else begin
			ecwi_print_info,ppar,pre,'existing file undisturbed',ofil,format='(a,a)'
			return
		endelse
	endif
	;
	; time stamp
	get_date,dstr,/time
	sxaddpar,hdr,'EDRPDATE',dstr,' ECWI DRP image write date'
	;
	; write it out
	mwrfits,img,ofil,hdr,iscale=iscale
	;
	; log
	ecwi_print_info,ppar,pre,'wrote image file',ofil,format='(a,a)'
	;
	return
end
