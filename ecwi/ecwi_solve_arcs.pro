...
;
; CALLING SEQUENCE:
;	ECWI_SOLVE_ARCS, Specs, Egeom, Ppar
;
; INPUTS:
...
;
; SIDE EFFECTS:
;	Modifies ECWI_GEOM struct by updating the bar coeffiecients and
...
;	2014-MAR-19	Use ECWI_CLEAN_COEFFS to fix errant bars
...
;	2014-SEP-11	Converted function to pro with status in egeom.status
...
;-
;
pro ecwi_solve_arcs, specs, egeom, ppar, tweak=tweak, plot_file=plot_file
...
ecwi_print_info,ppar,pre,'img, grat, filt, nasmsk, cwave', $
	imgnum,grating,filter,nasmask,cwvl, $
	format='(a,i6,2x,a-8,2x,a-8,i4,f12.3)'
;
; do initial fit of central third of ccd
ecwi_fit_center,specs,egeom,ppar,cntcoeff
;
; check status
if egeom.status gt 0 then begin
	ecwi_print_info,ppar,pre,'Cannot solve arcs',/error
	return
endif
;
; plot results
if ppar.display ge 1 then begin
	window,0,title='ecwi_solve_arcs'
	!p.multi=[0,1,2]
	si = 1.5
	...
	plot,cntcoeff[0,*],psym=4,charsi=si,charthi=th,thick=th, $
		...
		xthick=th,xtitle='Bar #', xrange=[-1,60],/xs, $
...
fwaves = fltarr(60,1000)
dwaves = fltarr(60,1000)
;
; keep track of reference wavelengths and matched pixel positions
rwaves = fltarr(60,1000)
xcents = fltarr(60,1000)
;
...
if keyword_set(tweak) then begin
	...
	if niter eq 1 then $
		ecwi_print_info,ppar,pre,'using '+strn(niter)+' iteration to measure RMS of initial central fit' $
	else	ecwi_print_info,ppar,pre,'using '+strn(niter)+' iterations to tweak higher order terms'
	;
	ecwi_print_info,ppar,pre,'Spectral line finding/matching thresh (frac. of res.): PkDel', $
	...
	if nqwvs eq 0 then begin
		ecwi_print_info,ppar,pre,'No wavelength overlap with atlas spectrum',/error
		egeom.status=5
		return
	endif
	...
	if twk_ref_npks eq 0 then begin
		ecwi_print_info,ppar,pre,'No good atlas points found',/error
		egeom.status=6
		return
	endif
	ecwi_print_info,ppar,pre,'Number of clean atlas lines found',twk_ref_npks,$
		format='(a,i4)'
	;
	if ppar.display ge 3 then begin
		...
		ecwi_legend,['PeakFit','WavRng'],linesty=[0,5],thick=[1.0,th], $
	...
	if ddisplay then $
		window,1,title='ecwi_solve_arcs(2)'
	;
	; now loop over the iterations and over the bars
	;
	if nasmask and niter gt 1 then $
		ecwi_print_info,ppar,pre,'tweaking not recommended when the NS mask is in',/warning
	;
	...
	for iter = 0,niter-1 do begin
		...
		for b=0, 59 do begin
			...
			slope_thresh = 0. ;0.7*smooth_width/2./100.0		; more severe for object
			ampl_thresh  = 25. ;egeom.rdnoise * 30.
			...
			if nmatchedpeaks le 0 then begin
				ecwi_print_info,ppar,pre,'No peaks matched in Bar # '+strn(b)+': '+strn(nmatchedpeaks),/warning
				barstat[b]=9
				goto, errbar
			endif
			;
			...
			if nmatchedpeaks le 2 then begin
				print,' '
				ecwi_print_info,ppar,pre,'Not enough peaks matched in Bar # '+strn(b)+': '+strn(nmatchedpeaks),/warning
				barstat[b]=9
				goto, errbar
			endif
			;
			...
			while nbad gt 0 do begin
				...
				if nmatchedpeaks le 0 then begin
					ecwi_print_info,ppar,pre,'All points rejected',/error
					barstat[b]=10
					goto, errbar
				endif
				;
			...
			if ddisplay and iter eq 0 then begin
				ecwi_legend,['Good','NoAtlas','Reject'],linesty=[0,0,0], $
		...
		ecwi_print_info,ppar,pre,"Iteration "+string(iter+1,"(i2)")+" of "+ $
		...
		if ppar.cleancoeffs and niter gt 1 then begin
			...
			if ppar.display ge 3 and clnplot then $
				window,2,title='ecwi_clean_coeffs'
			ecwi_clean_coeffs,twkcoeff,degree,ppar,plot=clnplot
			if ppar.display ge 3 and clnplot then $
				wdelete,2
		endif
	endfor; iter
...
if keyword_set(tweak) then begin
	...
	for b=0,59 do begin
		...
		if nmatchedpeaks gt 2 then begin
			...
			;
			; log stats
			ecwi_print_info,ppar,pre,ftype+' Fit: Bar#,Npks,RMS,Cdisp,Coefs', $
			...
			if display then begin
				...
				oplot,!x.crange,-[sigmas[b],sigmas[b]], $
					linesty=1,color=colordex('blue')
				ecwi_legend,["RMS = "+string(sigmas[b],form='(f7.3)')+' Ang', $
			...
			ecwi_print_info,ppar,pre,'Not enough matched peaks for bar',b,nmatchedpeaks,/error
...
if nbargood le 0 then begin
	ecwi_print_info,'No good bar fits',/error
	egeom.status = 10
	return
endif
;
...
y0wvs = fltarr(60)	; good wavelengths
y1wvs = fltarr(60)
t0wvs = fltarr(60)	; trim wavelengths
t1wvs = fltarr(60)
for b=0,59 do begin
	wy0 = poly(egeom.goody0,fincoeff[*,b])
	y0wvs[b] = wy0
	wy1 = poly(egeom.goody1,fincoeff[*,b])
	y1wvs[b] = wy1
	wt0 = poly(egeom.trimy0,fincoeff[*,b])
	t0wvs[b] = wt0
	wt1 = poly(egeom.trimy1,fincoeff[*,b])
	t1wvs[b] = wt1
endfor
;
...
;
; now let's update egeom struct
;
; check bar statuses
egeom.status = max(barstat)
if egeom.status gt 0 then begin
	ecwi_print_info,ppar,pre,'Some bars had issues, check log for details',/warning
	;
	; if N&S, then this was just for info, carry on
	if egeom.nasmask then $
		egeom.status=0
endif
;
; order of the fits
egeom.bfitord = degree+1
;
; reference bar coeff's
egeom.rbcoeffs = fincoeff[*,refbar]
;
; inclusive, all, and trim wavelengths
egeom.waveall0 = wavall0
egeom.waveall1 = wavall1
egeom.wavegood0 = wavgood0
egeom.wavegood1 = wavgood1
egeom.wavemid = wavmid
;
; make sure wavelengths are on fiducial scale and
; zeropoint is integer number of bins from fiducial wl
ndels = long((trim0 - egeom.wavefid)/egeom.dwout)
egeom.wave0out = egeom.wavefid + float(ndels) * egeom.dwout
ndels = long((trim1 - egeom.wavefid)/egeom.dwout)
egeom.wave1out = egeom.wavefid + float(ndels) * egeom.dwout
;
; log values
ecwi_print_info,ppar,pre,'min,max good wavelengths',egeom.wavegood0,egeom.wavegood1, $
	format='(a,2f9.2)'
ecwi_print_info,ppar,pre,'min,max inclusive wavelengths',egeom.waveall0,egeom.waveall1, $
	format='(a,2f9.2)'
ecwi_print_info,ppar,pre,'min,max trim wavelengths',egeom.wave0out,egeom.wave1out, $
	format='(a,2f9.2)'
ecwi_print_info,ppar,pre,'middle wavelength',egeom.wavemid,format='(a,f9.2)'
;
; set fit rms values
if keyword_set(tweak) then begin
	mom = moment(sigmas[bargood])
	egeom.avewavesig = mom[0]
	egeom.stdevwavesig = sqrt(mom[1])
	;
	; log results
	outliers = where(abs(sigmas[bargood]-mom[0]) gt 3.*sqrt(mom[1]), noutliers)
	ecwi_print_info,ppar,pre,'average bar wavelength sigma (Ang)',mom[0],' +- ',sqrt(mom[1]),$
		format='(a,f7.3,a,f7.3)'
	ecwi_print_info,ppar,pre,'min/max bar wavelength sigma (Ang)',minmax(sigmas[bargood]), $
		format='(a,f7.3,2x,f7.3)'
	if noutliers gt 0 then $
		ecwi_print_info,ppar,pre,'> 3sig outlier bars present, may want to tweak ppar.pkdel: Imgnum, Bars', $
...
for b=0,59 do begin
	;
	; update coeffs in egeom struct
	dcoeff = fincoeff[*,b]
	ecwi_apply_coeffs,egeom,b,dcoeff
	;
...
;
; stamp the ECWI_GEOM struct
egeom.progid = pre
;
; now let's make some plots!
;
; plot diagnostics of arc fits
ecwi_plot_arcfits,specs,egeom,ppar,cntcoeff,fincoeff,sigmas,fwaves,dwaves, $
