...
;
; CALLING SEQUENCE:
;	Result = ECWI_READ_PPAR( PPAR_FILENAME )
;
; INPUTS:
;	ppfname	- pipeline parameter file (written with ECWI_PPAR_WRITE)
;
; KEYWORDS:
;	VERBOSE - set this to get extra screen output
;
; RETURNS:
;	ECWI DRP parameter struct (as defined in ecwi_ppar__define.pro)
;
; PROCEDURE:
...
;		RAWROOT         '/Users/ecwi/drp/run/14/12/20'
...
;	parameters:
;
;	m82par = ECWI_READ_PPAR('m82.ppar')
;
; MODIFICATION HISTORY:
;	Written by:	Don Neill (neill@caltech.edu)
;	2013-APR-12	Initial version
;-
function ecwi_read_ppar,ppfname,verbose=verbose
;
; initialize
	pre = 'ECWI_READ_PPAR'

	A = {ecwi_ppar}		; get blank parameter struct
...
; use defaults, if name not passed in
	if n_params(0) lt 1 or n_elements(ppfname) le 0 then begin
		...
		if not file_test(ppfname,/read) then begin
			...
			if not file_test(ppfname,/read) then begin
				ecwi_print_info,ppar,pre,'default ppar file not found',/error
				return,ppar
			endif
		endif
	endif
