...
;
; CALLING SEQUENCE:
;	Result = ECWI_READ_IMAGE( ImgNo, Ppar, Tail, Hdr)
;
; Returns:
...
;	Ppar	- ECWI_PPAR struct specifying inputs and logging flags
...
;-
function ecwi_read_image,imgnum,ppar,tail,hdr, $
	...
	if n_params(0) lt 3 then begin
		if not keyword_set(calib) then begin
			ecwi_print_info,ppar,pre,'use ecwi_read_raw to read raw images',/error
			return,-1
		endif else tail = ''
	endif
	;
	...
	if keyword_set(calib) then $
		idir = ecwi_expand_dir(ppar.caldir) $
	else	idir = ecwi_expand_dir(ppar.reddir)
	if not file_test(idir,/directory,/executable,/read) then begin
		ecwi_print_info,ppar,pre,'input directory not accessible',idir,/error
		status = 2
		return,-1
	endif
	;
	...
	if keyword_set(master) then begin
		if strlen(master) gt 0 then begin
			...
			ecwi_print_info,ppar,pre,'invalid master file root',/error
			status = 3
			return,-1
		endelse
	;
	...
	if file_test(ifil) then begin
		...
		if strpos(tail,'msk') ge 0 then begin
			...
			ecwi_print_info,ppar,pre,'read mask file with no scaling',ifil,format='(a,a)'
		;
		; everything else should be floats
		endif else begin
			img = mrdfits(ifil,0,hdr,/silent,/fscale)
			ecwi_print_info,ppar,pre,'read image file with /fscale',ifil,format='(a,a)'
		endelse
		status = 0
	;
	; report non-existence
	endif else begin
		ecwi_print_info,ppar,pre,'file not found',ifil,/error
		status = 1
		img = -1
	endelse
	;
