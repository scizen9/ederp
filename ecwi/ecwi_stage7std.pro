...
;
; CALLING SEQUENCE:
;	ECWI_STAGE7STD, Pparfname, Linkfname
;
; OPTIONAL INPUTS:
...
;
;	ECWI_STAGE7STD,'night1/redux/ecwi.ppar'
;
...
;-
pro ecwi_stage7std,ppfname,linkfname,help=help,select=select, $
	...
	if keyword_set(proc_imgnums) then begin
		...
		if n_elements(proc_stdnums) ne nproc then begin
			ecwi_print_info,ppar,pre,'Number of stds must equal number of images',/error
			return
		endif
		imgnum = proc_imgnums
		snums = proc_stdnums
	...
	; if not use link file
		...
		;
		; read link file
		ecwi_read_links,ppar,linkfname,imgnum,std=snums,count=nproc, $
			select=select
	...
	;
	; log file
	lgfil = reddir + 'ecwi_stage7std.log'
	...
	for i=0,nproc-1 do begin
		...
		; image to process
		;
		; look for output from ecwi_stage6rr
		obfil = ecwi_get_imname(ppar,imgnum[i],'_icuber',/reduced)
		;
		...
		if file_test(obfil) then begin
			...
			;
			; final output file
			ofil = ecwi_get_imname(ppar,imgnum[i],'_icubes',/reduced)
			;
			; trim image type
			...
			if ppar.clobber eq 1 or not file_test(ofil) then begin
				...
				if snums[i] ge 0 then begin
					...
					stdf = ecwi_get_imname(ppar,snums[i],/nodir)
					;
					...
					if file_test(msfile) then begin
						...
						;
						; log that we got it
						ecwi_print_info,ppar,pre,'std file = '+msfile
					endif else begin
						;
						; does input std image exist?
						sinfile = ecwi_get_imname(ppar,snums[i],'_icuber',/reduced)
						;
						; if not check for raw cube
						if not file_test(sinfile) then $
							sinfile = ecwi_get_imname(ppar,imgnum[i],'_icube',/reduced)
						if file_test(sinfile) then begin
							do_std = (1 eq 1)
							ecwi_print_info,ppar,pre,'building std file = '+msfile
						endif else begin
							;
							; log that we haven't got it
							ecwi_print_info,ppar,pre,'std input file not found: '+sinfile,/warning
						endelse
					endelse
				endif
				;
				...
				if do_std then begin
					...
					if not file_test(msfile) then begin
						...
						scfg = ecwi_read_cfg(sinfile)
						;
						; build master std
						ecwi_make_std,scfg,ppar
					endif
					;
					; read in master std
					...
					ofil = ecwi_get_imname(ppar,imgnum[i],'_mcubes',/nodir)
					...
					ofil = ecwi_get_imname(ppar,imgnum[i],'_vcubes',/nodir)
					...
					ofil = ecwi_get_imname(ppar,imgnum[i],'_icubes',/nodir)
					...
					if file_test(sfil) then begin
						...
						;
						; correct extinction
						ecwi_correct_extin,sky,skyhdr,ppar
						;
						; do correction
						...
						;
						; write out final intensity image
						ofil = ecwi_get_imname(ppar,imgnum[i],'_scubes',/nodir)
					...
					if file_test(nfil) then begin
						...
						;
						; correct extinction
						ecwi_correct_extin,obj,objhdr,ppar
						;
						; do correction
						for is=0,11 do for ix = 0, sz[0]-1 do $
						...
						;
						; write out final intensity image
						ofil = ecwi_get_imname(ppar,imgnum[i],'_ocubes',/nodir)
						ecwi_write_image,obj,hdr,ofil,ppar
					endif
					;
					; handle the case when no std frames were taken
				endif else begin
					ecwi_print_info,ppar,pre,'cannot associate with any master std: '+ $
