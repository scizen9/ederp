...
;
; CALLING SEQUENCE:
;	ECWI_MAP_CCD, Hdr, Asec, Bsec, Csec, Tasec
;
; INPUTS:
;	Hdr	- FITS header for raw ECWI image
;
; KEYWORDS:
...
;-
pro ecwi_map_ccd, hdr, asec, bsec, csec, tsec, $
	namps = namps, $
	...
	for i = 0, namps-1 do begin
		;
		; ASEC
		as = ecwi_parse_imsec(sxpar(hdr,'ASEC'+strn(i+1)))
		asec[i,0,0] = (as[0] - 1) > 0
		...
		;
		; BSEC
		bs = ecwi_parse_imsec(sxpar(hdr,'BSEC'+strn(i+1)))
		bsec[i,0,0] = (bs[0] - 1) > 0
		...
		;
		; CSEC
		cs = ecwi_parse_imsec(sxpar(hdr,'CSEC'+strn(i+1)))
		csec[i,0,0] = (cs[0] - 1) > 0
