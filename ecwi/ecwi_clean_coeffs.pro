...
;
; CATEGORY:
;	Data reduction for the EMCCD Cosmic Web Images (ECWI).
;
; CALLING SEQUENCE:
;	ECWI_CLEAN_COEFFS, Coeffs, Forder, Ppar
;
; INPUTS:
;	Coeffs	- 9 x 120 array of spectral fit coefficients for each bar
;	Forder	- order of spectral fit (3 - 8)
;	Ppar	- ECWI_PPAR pipeline paramter struct
;
; OUTPUTS:
;	None
;
...
;
;-
pro ecwi_clean_coeffs,coeffs,forder,ppar,plot=plot
;
; startup
pre = 'ECWI_CLEAN_COEFFS'
q = ''
;
...
; check inputs
csz = size(coeffs,/dim)
if csz[0] lt forder+1 or csz[1] ne 60 then begin
	ecwi_print_info,ppar,pre, $
		'Coeffs dimension error, must be >= '+strn(forder+1)+ $
		' x 60, returning',/error
	return
endif
;
if forder le 0 then begin
	ecwi_print_info,ppar,pre,'Fit order must be greater than 0', $
		forder,/error,format='(a,2x,i6)'
	return
endif
;
if forder gt 8 or forder lt 1 then $
	ecwi_print_info,ppar,pre,'Fit order warning, should be in range 1 - 8',$
				...
				if display then begin
					...
					if i eq 0 then begin
						...
						oplot,!x.crange,[statrm[0,i,j]-statrm[1,i,j],statrm[0,i,j]-statrm[1,i,j]], $
							linesty=1
					endelse
					ecwi_legend,['Original','Fixed'],psym=[4,5],symsi=[2.,2.],charsi=1.5, $
						box=0,spac=2.5
					;
					; query user
...
if modified then begin
	...
	ecwi_print_info,ppar,pre,'No cleaning required.'
endelse
;
return
end		; ecwi_clean coeffs
