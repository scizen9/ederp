...
;
; CALLING SEQUENCE:
;	ECWI_QUICK, RawDir, ReducedDir, CalibDir, DataDir
;
; OPTIONAL INPUTS:
;	RawDir		- input raw directory (string) defaults to current dir
;	ReducedDir	- reduced data directory (string) defaults to './redux/'
;	CalibDir	- calib source directory (string) defaults to !ECWI_DATA + 'calib/'
;	DataDir		- ECWI data directory (string) defaults to !ECWI_DATA
;
; KEYWORDS:
...
;	for running ECWI_STAGE{N}.  Master calibration image pipeline parameter 
...
;-
pro ecwi_quick,rawdir,reduceddir,calibdir,datadir, $
	imgtarg=imgtarg, $
	froot=froot, $
	fdigits=fdigits, $
	...
	;
	; expand paths
	odir   = ecwi_expand_dir(odir)
	indir  = ecwi_expand_dir(indir)
	caldir = ecwi_expand_dir(caldir)
	ddir   = ecwi_expand_dir(ddir)
	;
	...
	; derive from file names in INDIR
		...
		if nf le 0 then begin
			ecwi_print_info,ppar,pre,'no fits files found in '+indir,/error
			return
		endif
		fdig = 0
		...
		ppar.fdigits = fdig
	endelse
	ecwi_print_info,ppar,pre,'number of digits in image numbers', ppar.fdigits
	;
	; create file spec
	...
	ecfg = ecwi_read_cfgs(indir,filespec=fspec)
	nf = n_elements(ecfg)
	;
	; check for requested object image
	if keyword_set(imgtarg) then begin
		it = where(ecfg.imgnum eq imgtarg, nit)
		if nit le 0 then begin
			ecwi_print_info,ppar,pre,'Requested image number not found',imgtarg,/error
			return
		endif
		if strpos(ecfg[it].imgtype,'object') lt 0 and $
		   strpos(ecfg[it].imgtype,'dflat') lt 0 then $
			ecwi_print_info,ppar,pre,'Requested image not an object/dflat image',imgtarg,ecfg[it].imgtype,/warn
	;
	; or just get most recent object image
	endif else begin
		ob = where(strpos(ecfg.imgtype,'object') ge 0 or $
			   strpos(ecfg.imgtype,'dflat') ge 0, nob)
		if nob le 0 then begin
			ecwi_print_info,ppar,pre,'No object/dflat images taken yet',/error
			return
		endif
		it = where(ecfg[ob].juliandate eq max(ecfg[ob].juliandate))
		it = [ob[it]]
	endelse
	;
	; check other params
	...
	;
	; log file
	lgfil = odir + 'ecwi_quick_'+strn(ecfg[it].imgnum)+'.log'
	...
	printf,ll,'Target Img: '+strn(ecfg[it].imgnum)
	...
	ecwi_print_info,ppar,pre,'Number of input images',nf
	;
	; trim imgtype tag
	ecfg.imgtype = strtrim(ecfg.imgtype,2)
	;
	; trim object name
	ecfg.object = strtrim(ecfg.object,2)
	;
	; exclude biases and test images from process list
	proc = where(strmatch(ecfg.imgtype,'bias') ne 1 and $
		     strmatch(ecfg.imgtype,'test') ne 1 and $
		     strmatch(ecfg.imgtype,'image') ne 1,nproc)
	;
	; if includetest set just exclude biases
	if ppar.includetest eq 1 then $
		proc = where(strmatch(ecfg.imgtype,'bias') ne 1,nproc)
	;
	; is there anything left to process?
	if nproc le 0 then begin
		ecwi_print_info,ppar,pre,'no object/cal images to process', $
			/error
		free_lun,ll
		return
	endif
	ecwi_print_info,ppar,pre,'Using only local calibrations'
	calcfg = ecfg
	ncal = nf
	ecwi_print_info,ppar,pre,'Number of images in calibration pool',ncal,$
		format='(a,i5)'
	;
	...
	if nprofs gt 0 then begin
		...
		ppar.profs = rlst
	endif else ecwi_print_info,ppar,pre,'no slice profile observations found',/warning
	;
	...
	if nrrs gt 0 then begin
		...
		ppar.rrs = rlst
	endif else ecwi_print_info,ppar,pre,'no relative response images found',/warning
	;
	; find sky observation images
	skys = where(ecfg.skyobs eq 1, nskys)
	if nskys gt 0 then begin
		ppar.skyexists = 1
		ppar.nskys = nskys
		rangepar,ecfg[skys].imgnum,rlst
		ppar.skys = rlst
	endif else ecwi_print_info,ppar,pre,'no sky observations found',/warning
	;
	; get image numbers to be processed
	imnums = ecfg[proc].imgnum
	rangepar,imnums,imlist
	ppar.imnum = imlist
	ppar.npims = nproc
	;
	; report
	ecwi_print_info,ppar,pre,'processing '+strtrim(strn(nproc),2)+' images'
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP BIASES
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	ecwi_group_biases,calcfg,ppar,bcfg
	;
	; do we have any bias groups?
	if ppar.nbgrps le 0 then $
		ecwi_print_info,ppar,pre,'no bias groups found',/warning
	ecwi_print_info,ppar,pre,'number of bias groups = '+ $
		strtrim(string(ppar.nbgrps),2)
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP DARKS
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	ecwi_group_darks,calcfg,ppar,dcfg
	;
	; do we have any dark groups?
	if ppar.ndgrps le 0 then $
		ecwi_print_info,ppar,pre,'no dark groups found',/warning
	ecwi_print_info,ppar,pre,'number of dark groups = '+ $
		strtrim(string(ppar.ndgrps),2)
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP FLATS
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	ecwi_group_flats,calcfg,ppar,fcfg
	;
	; do we have any flat groups?
	if ppar.nfgrps le 0 then $
		ecwi_print_info,ppar,pre,'no flat groups found',/warning
	ecwi_print_info,ppar,pre,'number of flat groups = '+ $
		strtrim(string(ppar.nfgrps),2)
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP CBARS AND ARC FILES (GEOM)
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	ecwi_group_geom,calcfg,ppar,ccfg,acfg,ngeom
	ppar.ncbars = ngeom
	ppar.narcs = ngeom
	ecwi_print_info,ppar,pre,'number of cbars, arc images',ngeom
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if nprofs gt 0 then $
		pcfg = calcfg[profs]
	ecwi_print_info,ppar,pre,'number of slice profile images',nprofs
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if nskys gt 0 then $
		scfg = ecfg[skys]
	ecwi_print_info,ppar,pre,'number of sky images',nskys
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if nrrs gt 0 then $
		rcfg = calcfg[rrs]
	ecwi_print_info,ppar,pre,'number of relative response images',nrrs
	;
	; write out master ECWI_PPAR into file
	ppar.ppfname = 'ecwi.ppar'
	ecwi_write_ppar,ppar,/archive
	;
	; now set input dir to output for subsequent stages
	ppar.reddir = odir
	;
	; set up configuration matching: here is our list of 
	; default tags to match in the ECWI_CFG struct
	...
	for i=0,nproc-1 do begin
		...
		;
		; get image summary
		ecwi_print_cfgs,ecfg[p],imsum,/silent
		;
		; format for output
		...
		if ppar.nbgrps gt 0 then begin
			mcfg = ecwi_associate(bcfg,ecfg[p],ppar,count=b)
			if b eq 1 then begin
				mbfile = mcfg.groupfile
				blink = mcfg.groupnum
				;
				; log
				ecwi_print_info,ppar,pre,'master bias file = '+$
					mbfile
			;
		...
		if ppar.ndgrps gt 0 then begin
			mcfg = ecwi_associate(dcfg,ecfg[p],ppar,count=d)
			if d eq 1 then begin
				mdfile = mcfg.groupfile
				dlink = mcfg.groupnum
				;
				; log
				ecwi_print_info,ppar,pre,'master dark file = '+$
					mdfile
			;
		...
		if strmatch(ecfg[p].imgtype,'dark') ne 1 and ppar.nfgrps gt 0 then begin
			mcfg = ecwi_match_cfg(fcfg,ecfg[p],ppar,mtags,imgtype='cflat',/time,count=f,/silent)
			if f eq 1 then begin
				mffile = mcfg.groupfile
				flink = mcfg.groupnum
				;
				; log
				ecwi_print_info,ppar,pre,'master flat file = ' + mffile
			;
		...
		if strmatch(ecfg[p].imgtype,'dark') ne 1 and ppar.ncbars gt 0 and ppar.narcs gt 0 then begin
			mcfg = ecwi_match_cfg(ccfg,ecfg[p],ppar,mtags,imgtype='cbars',/time,count=c,/silent)
			if c eq 1 then begin
				...
				;
				; log
				ecwi_print_info,ppar,pre,'cbars file = '+cbfile
				ecwi_print_info,ppar,pre,'arc   file = '+arfile
				;
		...
		if strmatch(ecfg[p].imgtype,'dark') ne 1 and ppar.nprofs gt 0 and $
			links[icbar] ge 0 and links[iarc] ge 0 then begin
			mcfg = ecwi_match_cfg(pcfg,ecfg[p],ppar,mtags,count=s,/time)
			if s eq 1 then begin
				...
				;
				; log
				ecwi_print_info,ppar,pre,'slice profile file = '+pfile
				;
		...
		if strmatch(ecfg[p].imgtype,'object') eq 1 and $
		   ecfg[p].skyobs eq 0 and ppar.nskys gt 0 then begin
			mcfg = ecwi_match_cfg(scfg,ecfg[p],ppar,mtags,/object,/time,count=s,/silent)
			if s eq 1 then begin
				;
				; record sky filename
				...
				;
				; log
				ecwi_print_info,ppar,pre,'sky file = '+skfile
			endif else begin
				ecwi_print_info,ppar,pre,'No sky obs with same object name', $
					ecfg[p].object,format='(a,2x,a)',/warning
				slink = -1
			endelse
			;
			; set sky link
		...
		; also require geometry solution
		if strmatch(ecfg[p].imgtype,'dark') ne 1 and ppar.nrrs gt 0 and $
			links[icbar] ge 0 and links[iarc] ge 0 then begin
			mcfg = ecwi_match_cfg(rcfg,ecfg[p],ppar,mtags,count=r,/time)
			if r eq 1 then begin
				...
				;
				; log
				ecwi_print_info,ppar,pre,'relative response file = '+rrfile
			endif else begin
				rlink = -1
			endelse
			;
		...
		;
		; write out links
		printf,kl,ecfg[p].imgnum,links,imsum,format='(10i9,2x,a)'
	...
	ecwi_read_links,ppar,linkfname,inums,bias=bnums,dark=dnums,flat=fnums, $
		cbar=cnums,arc=anums,prof=pnums,rrsp=rnums
	;
	; target image pointer
	ti = (where(inums eq ecfg[it].imgnum, nti))[0]
	if nti le 0 then begin
		ecwi_print_info,ppar,pre,'Target image not found in link file',/error
		return
	endif
	;
	...
	if dnums[ti] ge 0 then begin
		dpar = ecwi_read_ppar(odir+'mdark_'+strn(dnums[ti])+'.ppar')
	...
	if fnums[ti] ge 0 then begin
		fpar = ecwi_read_ppar(odir+'mflat_'+strn(fnums[ti])+'.ppar')
	...
	;
	; do stage 1
	ecwi_stage1,proc_imgnums=s1img,proc_biasnums=s1bias
	stagetail[1] = 'int.fits'
	;
	; do stage 2 dark
	if dnums[ti] ge 0 then begin
		ecwi_stage2dark,proc_imgnums=s1img,proc_darknums=s2dark
		stagetail[2] = 'intd.fits'
	endif
	;
	; do stage 3 flat
	if fnums[ti] ge 0 then begin
		ecwi_stage3flat,proc_imgnums=s1img,proc_flatnums=s3flat
		stagetail[3] = 'intf.fits'
	endif
	;
	; do stage 4 geom
	if cnums[ti] ge 0 and anums[ti] ge 0 then begin
		ecwi_stage4geom,proc_imgnums=s1img,proc_cbarnums=s4cbars,proc_arcnums=s4arcs
		stagetail[4] = 'icube.fits'
	endif
	;
	; do stage 5 profile
	if pnums[ti] ge 0 then begin
		ecwi_stage5prof,proc_imgnums=s1img,proc_profnums=s5profs
	...
	if pnums[ti] ge 0 and rnums[ti] ge 0 then begin
		ecwi_stage6rr,proc_imgnums=s1img,proc_rrnums=s6rrs
	...
	if lstage ge 4 then begin
		...
		zrat = egeom.slscl/(egeom.pxscl*egeom.xbinsize)
		;
		; pick correct tail
	...
	if keyword_set(ds9) then begin
		ecwi_print_info,ppar,pre,'Displaying... ',ds9cmd, $
			format='(a,a)'
		spawn,ds9cmd
	endif
	;
