...
;	for the ECWI Data Reduction Pipeline.
;
; CATEGORY:
...
;
; SIDE EFFECTS:
;	defines the ECWI_PPAR data structure
;
; PROCEDURE:
;	Provide the automatic structure definition for the ECWI pipeline
;	parameter structure ECWI_PPAR.
;
; EXAMPLE:
;	Generate a Ppfile called 'm82.ppar' for the red side images
;	in directory 'night1' and put it in 'night1/redux':
;
;	ECWI_INIT,'M82.ppar','night1','night1/redux','red*.fit*',ppar=KPP
;
;	KPP will be initialized to have the correct pipeline parameters for
;	the inputs to ECWI_INIT.
;
; NOTES:
...
;-
pro ecwi_ppar__define
;
tmp = { ecwi_ppar, $
;
; output filename
	ppfname:'ecwi.ppar', $	; pipeline parameter set name, e.g. 'm81.ppar'
	lnfname:'ecwi.link', $	; processing links file name, e.g. 'm81.link'
;
...
; Slice profile processing
	slicex0:19, $		; safe lower spatial limit for slices (unbin px)
	slicex1:172, $		; safe upper spatial limit for slices (unbin px)
