...
;	ECWI_CFG struct, then the output rr image would have the
...
;-
pro ecwi_slice_rr,ecfg,ppar,rr
	;
	; setup
	pre = 'ECWI_SLICE_RR'
	;
	...
	if strmatch(strtrim(ecfg.imgtype,2),'dflat') eq 0 and $
	   strmatch(strtrim(ecfg.imgtype,2),'tflat') eq 0 then begin
		ecwi_print_info,ppar,pre,'not an rr obs',/warning
	endif
	;
	; directories
	...
	ofil = ecwi_get_imname(ppar,ecfg.imgnum,'_rr',/reduced)
	...
	icub = ecwi_read_image(ecfg.imgnum,ppar,'_icubep',hdr,/calib,status=stat)
	;
	...
	;
	; log results
	ecwi_print_info,ppar,pre,'Resp. Pars: X0, X1, Y0, Y1, Wav0, Wav1', $
	...
	if doplots ge 2 then begin
		...
		plot,w[y0:y1],refr,xthick=th,ythick=th,charsi=si,charthi=th, $
			title='Img: '+strn(ecfg.imgnum)+' Reference Slice: '+strn(irs), $
	...
	for i=0,11 do begin
		...
		;
		; log results
		ecwi_print_info,ppar,pre,'Resp. Fit: Slice#, Coefs',i,fco, $
		...
		if doplots ge 2 then begin
			...
			plot,w[y0:y1],nrr,xthick=th,ythick=th,charsi=si, $
				charthi=th, $
				title='Img: '+strn(ecfg.imgnum)+' Slice: '+strn(i)+rlab, $
	...
	sxaddpar,hdr,'CNAME1','ECWI DEC',' DEC name'
	...
	sxaddpar,hdr,'CNAME2','ECWI RR Wavelength',' Wavelength name'
	...
	ofil = ecwi_get_imname(ppar,ecfg.imgnum,'_rr',/nodir)
	ecwi_write_image,rrs,hdr,ofil,ppar
	return
end
