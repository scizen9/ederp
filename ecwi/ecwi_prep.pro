...
;
; CALLING SEQUENCE:
;	ECWI_PREP, RawDir, ReducedDir, CalibDir, DataDir
;
; OPTIONAL INPUTS:
...
;		1) a master pipeline parameter file, ecwi.ppar, with paramters
;			for running the pipeline.
;		2) a master link file, ecwi.link, with associations for running 
;			subsequent stages of the pipeline (ECWI_STAGE{N}).
...
;		4) a general image log file, ecwi.imlog, that records the 
;			configuration of each input raw image.  
;		5) a log file, ecwi_prep.log, that logs all steps executed in 
;			this stage and records execution time.
; NOTE:
;	The ecwi.link file can be edited to override the automated associations
...
;
; EXAMPLE:
;	Prepare to perform ECWI reductions on the images in 'night1' 
...
;	2013-SEP-13	Use ECWI_PPAR struct for subroutine parameters
...
;	2014-APR-01	Added ecwi_group_geom and processing of all calib imgs
...
;-
pro ecwi_prep,rawdir,reduceddir,calibdir,datadir, $
	froot=froot, $
	fdigits=fdigits, $
	...
	nf = n_elements(ecfg)
	ecwi_print_info,ppar,pre,'Number of raw input images',nf
	;
	; write out a complete listing
	ecwi_print_cfgs,ecfg,/silent,/header,outfile=odir+'ecwi.imlog'
	;
	; trim imgtype tag
	...
	;calcfg = ecwi_read_cfgs(caldir,filespec=fspec,count=ncal,/silent)
	;
	; combine them with local calibrations which take precedence
	;if ncal gt 0 then begin
	;	ecwi_print_info,ppar,pre,'Including library calibrations from',caldir
	;	calcfg = [calcfg,ecfg]
	;
	...
	if ntrrs gt 0 or ndrrs gt 0 then begin
		...
		nrrs = 0
		ecwi_print_info,ppar,pre,'no relative response images found',/warning
	endelse
	;
	...
	stds = ecwi_find_stds(ecfg,ppar,nstds)
	if nstds le 0 then $
		ecwi_print_info,ppar,pre,'no standard star images found',/warning
	;
	...
	ecwi_print_info,ppar,pre,'Number of bias groups', ppar.nbgrps
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP DARKS
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	ecwi_print_info,ppar,pre,'Number of dark groups', ppar.ndgrps
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	; GROUP FLATS
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	ecwi_print_info,ppar,pre,'Number of flat groups', ppar.nfgrps
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if ppar.ncbars le 0 then $
		ecwi_print_info,ppar,pre,'no geom groups found',/warning
	ecwi_print_info,ppar,pre,'Number of geom groups', ppar.ncbars
	;
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	if nstds gt 0 then $
		stdcfg = ecfg[stds]
	ecwi_print_info,ppar,pre,'Number of standard star images',nstds
	;
	...
	filestamp,odir+'ecwi.link',/arch
	openw,kl,odir+'ecwi.link',/get_lun
	...
	for i=0,nproc-1 do begin
		...
		if strmatch(ecfg[p].imgtype,'dark') ne 1 and ppar.nfgrps gt 0 then begin
			mcfg = ecwi_match_cfg(fcfg,ecfg[p],ppar,mtags,imgtype='cflat',/time,count=f,/silent,cwi=cwi)
		...
		if strmatch(ecfg[p].imgtype,'dark') ne 1 and ppar.ncbars gt 0 and ppar.narcs gt 0 then begin
			mcfg = ecwi_match_cfg(ccfg,ecfg[p],ppar,mtags,imgtype='cbars',/time,count=c,/silent,cwi=cwi)
		...
		if strmatch(ecfg[p].imgtype,'dark') ne 1 and ppar.nprofs gt 0 and $
			...
			mcfg = ecwi_match_cfg(pcfg,ecfg[p],ppar,mtags,count=s,/time,cwi=cwi)
		...
		if strmatch(ecfg[p].imgtype,'object') eq 1 and $
		   ecfg[p].skyobs eq 0 and ppar.nskys gt 0 then begin
			mcfg = ecwi_match_cfg(scfg,ecfg[p],ppar,mtags,/object,/time,count=s,/silent,cwi=cwi)
			if s eq 1 then begin
				;
				; record sky filename
		...
		if strmatch(ecfg[p].imgtype,'dark') ne 1 and ppar.nrrs gt 0 and $
			...
			; twilight flats
			if ntrrs gt 0 then $
				mtcfg = ecwi_match_cfg(rtcfg,ecfg[p],ppar,mtags,count=rt,/time) $
			else	rt = 0
			;
			; dome flats
			if ndrrs gt 0 then $
				mdcfg = ecwi_match_cfg(rdcfg,ecfg[p],ppar,mtags,count=rd,/time) $
			else	rd = 0
			;
			; do we have a choice?
		...
		if nstds gt 0 and strmatch(ecfg[p].imgtype,'object') eq 1 and links[irrsp] ge 0 then begin
			mcfg = ecwi_match_cfg(stdcfg,ecfg[p],ppar,mtags,count=std,/time)
			if std eq 1 then begin
				...
				;
				; log
				ecwi_print_info,ppar,pre,'standard star observation file = '+stdfile
			endif else begin
				stdlink = -1
			endelse
			;
