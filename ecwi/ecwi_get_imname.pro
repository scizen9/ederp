function ecwi_get_imname,ppar,imgnum,tail, $
	...
	if n_params(0) lt 2 or keyword_set(help) then begin
		print,pre+': Info - Usage: full_filename = '+pre+'( EcwiPpar, ImgNum, [Tail], /CALIB, /INPUT, /OUTPUT, /MASTER)'
		return,''
	endif
	;
	; verify ppar
	...
	if imgnum lt 0 then begin
		ecwi_print_info,ppar,pre,'invalid image number',/error
		return,''
	endif
	;
	; check tail
