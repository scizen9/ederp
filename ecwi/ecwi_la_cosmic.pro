...
;
; CALLING SEQUENCE:
;   ecwi_la_cosmic, img, mask, [sigclip=, gain=, readn=, $
...
;   17-SEP-2013  Now use ppar to pass loglun, verbose and use ecwi_print_info
;-
;------------------------------------------------------------------------------
pro ecwi_la_cosmic, img, ppar, mask, sigclip=sigclip, $
...
;; ppar
if ecwi_verify_ppar(ppar) ne 0 then $
	ppar = { ecwi_ppar }
;; timing
tstart = systime(1)
...
if ppar.verbose ge 1 then begin
	...
	print, " IDL version by Josh Bloom, ECWI version by Don Neill"
endif
ecwi_print_info,ppar,pre,systime(0)
ecwi_print_info,ppar,pre,'initial gain, readnoise: ' + $
		strtrim(string(gain),2)+', '+strtrim(string(readn),2)
ecwi_print_info,ppar,pre,'Obj thresh, sigma clip : ' + $
...
while(not sstop) do begin
	...
	if (gain le 0.0) then begin
		...
		ecwi_print_info,ppar,pre,"Approximate sky level = " + $
				strtrim(string(skylev),2) + ' ADU'
		ecwi_print_info,ppar,pre,"Sigma of sky = " + strtrim(string(sig),2)
		ecwi_print_info,ppar,pre,"Estimated gain = " + strtrim(string(usegain),2)
		if (usegain le 0) then begin
			ecwi_print_info,ppar,pre,'Gain was found to be less than zero',/error
			ecwi_print_info,ppar,pre,'is it possible you forgot to give a "skyval"?',/error
			return
		endif
	...
	ecwi_print_info,ppar,pre,'found ' + strtrim(string(ncosmicray)) + ' cosmic rays in iteration ' + strtrim(string(iter)),info=2
...
ecwi_print_info,ppar,pre,'found total of ' + strtrim(string(ntcosmicray)) + $
