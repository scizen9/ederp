...
;
; CALLING SEQUENCE:
;	ECWI_MAKE_STD, Ecfg,  Ppar, Invsen
;
; INPUTS:
;	Ecfg	- ECWI_CFG struct for the input data cube, preferrably
...
;	input ECWI_CFG struct, then the output std image would have the
...
;-
pro ecwi_make_std,ecfg,ppar,invsen
	;
	; setup
	pre = 'ECWI_MAKE_STD'
	q=''
	;
	; check inputs
	if ecwi_verify_cfg(ecfg,/init) ne 0 then return
	...
	ofil = ecwi_get_imname(ppar,ecfg.imgnum,'_invsens',/reduced)
	if file_test(ofil) then begin
		if ppar.clobber ne 1 then begin
			ecwi_print_info,ppar,pre,'output file already exists',ofil,/error
			return
		endif else $
			ecwi_print_info,ppar,pre,'output file will be overwritten',ofil,/warning
	endif
	;
	; read in image
	icub = ecwi_read_image(ecfg.imgnum,ppar,'_icuber',hdr,/calib,status=stat)
	if stat ne 0 then begin
		icub = ecwi_read_image(ecfg.imgnum,ppar,'_icube',hdr,/calib,status=stat)
		if stat ne 0 then begin
	...
	spath = !ECWI_DATA + '/stds/'+sname+'.fits'
	if not file_test(spath) then begin
		ecwi_print_info,ppar,pre,'standard star data file not found for: '+sname,/error
		return
	endif
	ecwi_print_info,ppar,pre,'generating effective inverse sensitivity curve from '+sname
	;
	; get size
	sz = size(icub,/dim)
	;
	...
	if expt eq 0. then begin
		ecwi_print_info,ppar,pre,'no exposure time found, setting to 1s',/warn
		expt = 1.
	endif else $
		ecwi_print_info,ppar,pre,'Using exposure time of',expt,/info
	;
	...
	;
	; good spatial range
	gx0 = ppar.slicex0/ecfg.xbinsize
	gx1 = ppar.slicex1/ecfg.xbinsize
	x = indgen(sz[0])
	;
	;
	; log results
	ecwi_print_info,ppar,pre,'Invsen. Pars: X0, X1, Y0, Y1, Wav0, Wav1', $
	...
	;
	; log results
	ecwi_print_info,ppar,pre,'Std slices; max, sl0, sl1, spatial cntrd', $
	...
	ecwi_correct_extin, scub, hdr, ppar
	;
	; get slice spectra
	...
	if nsroi le 0 then begin
		ecwi_print_info,ppar,pre,'no standard wavelengths in common',/error
		return
	;
	...
	sfw = sfw[sroi]
	fwhm = max(sfw)
	ecwi_print_info,ppar,pre,'reference spectrum FWHM used',fwhm,'Angstroms', $
		format='(a,f5.1,1x,a)'
	;
	...
	if nt gt 0 then begin
		...
		ecwi_print_info,ppar,pre,'no good wavelengths to fit',/error
	endelse
	;
	; plot effective inverse sensitivity
	if doplots then begin
		yrng = get_plotlims(invsen[gy])
		plot,w,invsen,title=sname+' Img #: '+strn(ecfg.imgnum), $
			xtitle='Wave (A)',xran=[wall0,wall1],/xs, $
		...
		if area gt 0 then begin
			plot,w,earea, $
				title=sname+' Img #: '+strn(ecfg.imgnum)+tlab, $
				xtitle='Wave (A)',xran=[wall0,wall1],/xs, $
			...
			plot,w,earea,title=sname+' Img #: '+strn(ecfg.imgnum), $
				xtitle='Wave (A)',xran=[wall0,wall1],/xs, $
	...
	sxaddpar,hdr,'CNAME1','ECWI INVSENS Wavelength',' Wavelength name'
	...
	ofil = ecwi_get_imname(ppar,ecfg.imgnum,'_invsens',/nodir)
	ecwi_write_image,invsen,hdr,ofil,ppar
	;
	...
	sxaddpar,hdr,'CNAME1','ECWI EA Wavelength',' Wavelength name'
	;
	; write out effective area file
	ofil = ecwi_get_imname(ppar,ecfg.imgnum,'_ea',/nodir)
	ecwi_write_image,earea,hdr,ofil,ppar
	;
	return
end
