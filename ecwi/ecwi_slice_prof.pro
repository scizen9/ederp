...
;
; CALLING SEQUENCE:
;	ECWI_SLICE_PROF, Ecfg,  Ppar, Prof
;
; INPUTS:
...
;	ECWI_CFG struct, then the output profile image would have the
...
;	2013-NOV-12	Put spatial inclusive limits in ECWI_PPAR struct (ppar)
;	2014-APR-08	Check against zero-like values in profile causing overflow
;-
pro ecwi_slice_prof,ecfg,ppar,profs
	;
	; setup
	pre = 'ECWI_SLICE_PROF'
	;
	...
	if ecfg.skyobs eq 0 and ecfg.shuffmod eq 0 then begin
		ecwi_print_info,ppar,pre,'not a sky obs',/warning
	endif
	;
	; what kind are we?
	if ecfg.shuffmod eq 1 then $
		ecwi_print_info,ppar,pre,'nod-and-shuffle sky obs',/info
	if ecfg.skyobs eq 1 then $
		ecwi_print_info,ppar,pre,'standard sky obs',/info
	;
	; directories
	...
	ofil = ecwi_get_imname(ppar,ecfg.imgnum,'_prof',/reduced)
	...
	if ecfg.nasmask eq 1 then begin
		if ecfg.shuffmod eq 1 then begin
			;
			; read sky cube from nod-and-shuffle image set
			icub = ecwi_read_image(ecfg.imgnum,ppar,'_scube',hdr,status=stat)
			if stat ne 0 then return
		endif else begin
			;
			; read sky observation image
			icub = ecwi_read_image(ecfg.imgnum,ppar,'_icube',hdr,status=stat)
			if stat ne 0 then return
		endelse
		;
		; get size
	...
	;
	; log
	if ecfg.nasmask ne 1 then $
		ecwi_print_info,ppar,pre,':Slice   Bl:Mn    Sg       Gr:Mn    Sg       Rd:Mn    Sg       All:Mn   Sg' $
	else	ecwi_print_info,ppar,pre,':Slice   All:Mn   Sg'
	;
	; display status
	...
	for i=0,11 do begin
		...
		if ecfg.nasmask ne 1 then begin
			;
			...
			mn3 = mo3[0]
			sg3 = sqrt(mo3[1])
			ecwi_print_info,ppar,pre,'',i,mn1,sg1,mn2,sg2,mn3,sg3,mn,sg, $
				format='(a,i4,2x,8f9.4)'
		endif else $
			ecwi_print_info,ppar,pre,'',i,mn,sg, $
		...
		if doplots ge 2 then begin
			...
			plot,prof,xthick=th,ythick=th,charsi=si,charthi=th, $
				title='Image # '+strn(ecfg.imgnum)+', Slice '+strn(i),psym=10, $
				xtitle='Pixel',/xs, $
	...
	ofil = ecwi_get_imname(ppar,ecfg.imgnum,'_prof',/nodir)
	ecwi_write_image,profs,hdr,ofil,ppar
	return
end
