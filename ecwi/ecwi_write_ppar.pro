...
;		RAWDIR          '/Users/ecwi/drp/run/14/12/20'
;		/ root dir of raw data
;		GAIN1                   0.14500 / gain for amp 1 in electrons/DN
;
;	The details of the processing steps can be found in the ECWI DRP 
;	documentation and are briefly described in ECWI_PIPE.PRO.
;
; EXAMPLE:
;
...
;-
pro ecwi_write_ppar,ppar,force=force,archive=archive
;
; initialize
	pre = 'ECWI_WRITE_PPAR'
...
; don't write out if the file name is not set
	if strlen(ppar.ppfname) le 0 then begin
		ecwi_print_info,ppar,pre,'PPFNAME tag empty',/error
		return
	endif
; don't write out if it isn't initialized
	if not ppar.initialized and not keyword_set(force) then begin
		ecwi_print_info,ppar,pre,'ECWI_PPAR struct not initialized',/error
		return
	endif
;
; test filename
	...
	if file_test(ofile) then begin
		if keyword_set(archive) then begin
			...
			ecwi_print_info,ppar,pre,'archived existing ppar file', $
				ofile,format='(a,a)'
		endif else begin
			file_delete,ofile,verbose=ppar.verbose
			ecwi_print_info,ppar,pre,'deleted existing ppar file', $
				ofile,format='(a,a)'
		endelse
	endif
;
...
; not a legal parameter
			...
			ecwi_print_info,ppar,pre,'not printing ',keys[i],/warning
	endfor
;
; close up
	free_lun,ol
	ecwi_print_info,ppar,pre,ppar.ppfname + ' written in directory ' + $
		ppar.reddir
;
; restore loglun
	ppar.loglun = loglun
