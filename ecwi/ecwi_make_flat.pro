...
;	image numbers given in ECWI_PPAR struct, then creates a flat image
...
;
; INPUTS:
;	Ppar	- ECWI_PPAR struct for flat group to combine
;
; KEYWORDS:
;	None
;
...
; EXAMPLE:
;
;	ppar = ECWI_PPAR_READ('mflat_0.ppar')
;	ECWI_MAKE_FLAT,ppar
;
; TODO:
...
pro ecwi_make_flat,ppar
	...
	if strlen(ppar.cflats) le 0 then begin
		ecwi_print_info,ppar,pre,'no flats entered',/error
		return
	endif
	;
	; directories
	...
	ecwi_print_info,ppar,pre,'combining this many images',nf
	;
	; read first image
	...
	if file_test(ffil) then begin
		...
		ecwi_print_info,ppar,pre,'Image file not found',ffil, $
			format='(a,a)',/error
		return
	endelse
	;
	...
	if nf eq 2 then begin
		...
		if sz[0] ne nx or sz[1] ne ny then begin
			ecwi_print_info,ppar,pre,'Size mismatch',ffil,/error
			return
		endif
		;
		; simple average
	...
	;
	; now fit master flat
	ecwi_fit_flat,mflat,hdr,ppar,flato
	;
	...
	ecwi_write_image,flato,hdr,ppar.masterflat,ppar
	;
	return
end
