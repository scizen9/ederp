...
;	image numbers given in ECWI_PPAR struct PPAR.BIASES.
;
; CATEGORY:
...
;
; INPUTS:
;	ppar	- ECWI_PPAR struct for bias group to combine
;
; KEYWORDS:
;	None
;
...
;
;	Ppar = ECWI_READ_PPAR('redux/mbias_0.ppar')
;	ECWI_MAKE_BIAS,Ppar
;
; MODIFICATION HISTORY:
;	Written by:	Don Neill (neill@caltech.edu)
;	2013-APR-22	Initial version
;	2013-MAY-08	remove SKIP1 keyword and now use ECWI_PPAR as input
;	2013-JUL-03	added logging
;	2013-SEP-14	use ppar to pass loglun
;-
pro ecwi_make_bias,ppar
	;
	; initialize
	pre = 'ECWI_MAKE_BIAS'
	;
	...
	if strlen(ppar.biases) le 0 then begin
		ecwi_print_info,ppar,pre,'no biases entered',/error
		return
	endif
	;
	; get image list
	...
	if nb ge ppar.mingroupbias then begin
		...
		ecwi_print_info,ppar,pre,'combining this many images',nb
		;
		; set up image stack
		im = ecwi_read_raw(ppar,bnums[0],header=hdr,/silent)
		nx = sxpar(hdr,'NAXIS1')
		ny = sxpar(hdr,'NAXIS2')
		...
		for i=1,nb-1 do begin
			im = ecwi_read_raw(ppar,bnums[i],header=bhdr, $
					/silent)
			if strtrim(sxpar(bhdr,'IMGTYPE'),2) eq 'bias' then begin
				stack[nstack,*,*] = im
				nstack = nstack + 1
			endif else begin
				ecwi_print_info,ppar,pre,'not a bias image', $
					bnums[i],/warning
				bnums[i] = -1
			endelse
		endfor
		...
		ecwi_print_info,ppar,pre,'Master Bias has '+strn(nstack)+ $
			' images'
		;
		...
		ecwi_map_ccd,hdr,asec,bsec,csec,tsec,namps=namps,verbose=ppar.verbose
		...
		for ia = 0, namps - 1 do begin
			...
			;
			; log
			ecwi_print_info,ppar,pre,'Amp'+strn(ia+1)+ $
		...
		ecwi_write_image,mbias,hdr,ofile,ppar
	endif else $
		ecwi_print_info,ppar,pre,'too few bias frames',nb,/error
	;
	return
end
