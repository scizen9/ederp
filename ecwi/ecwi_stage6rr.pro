...
;
; PURPOSE:
;	This procedure takes the output from ECWI_STAGE5PROF and 
...
;
; CALLING SEQUENCE:
;	ECWI_STAGE6RR, Pparfname, Linkfname
;
; OPTIONAL INPUTS:
...
;
;	ECWI_STAGE6RR,'night1/redux/rr.ppar'
;
...
;-
pro ecwi_stage6rr,ppfname,linkfname,help=help,select=select, $
	...
	if keyword_set(proc_imgnums) then begin
		...
		if n_elements(proc_rrnums) ne nproc then begin
			ecwi_print_info,ppar,pre,'Number of rrs must equal number of images',/error
			return
		endif
		imgnum = proc_imgnums
		rnums = proc_rrnums
	...
	; if not use link file
		...
		;
		; read link file
		ecwi_read_links,ppar,linkfname,imgnum,rrsp=rnums,count=nproc, $
			select=select
	...
	;
	; log file
	lgfil = reddir + 'ecwi_stage6rr.log'
	...
	for i=0,nproc-1 do begin
		...
		obfil = ecwi_get_imname(ppar,imgnum[i],'_icubep',/reduced)
		;
		...
		if file_test(obfil) then begin
			...
			;
			; final output file
			ofil = ecwi_get_imname(ppar,imgnum[i],'_icuber',/reduced)
			;
			; trim image type
			...
			if ppar.clobber eq 1 or not file_test(ofil) then begin
				...
				if rnums[i] ge 0 then begin
					;
					; master rr file name
					rrf = ecwi_get_imname(ppar,rnums[i],/nodir)
					;
					...
					if file_test(mrfile) then begin
						...
						;
						; log that we got it
						ecwi_print_info,ppar,pre,'slice rr file = '+mrfile
					endif else begin
						;
						; does input rr image exist?
						;
						; check for profile corrected cube first
						rinfile = ecwi_get_imname(ppar,rnums[i],'_icubep',/reduced)
						;
						; if not check for data cube
						if not file_test(rinfile) then $
							rinfile = ecwi_get_imname(ppar,rnums[i],'_icube',/reduced)

						if file_test(rinfile) then begin
							do_rr = (1 eq 1)
							ecwi_print_info,ppar,pre,'building slice rr file = '+mrfile
						endif else begin
							;
							; log that we haven't got it
							ecwi_print_info,ppar,pre,'slice rr input file not found: '+rinfile,/warning
						endelse
					endelse
				endif
				;
				...
				if do_rr then begin
					...
					if not file_test(mrfile) then begin
						...
						rcfg = ecwi_read_cfg(rinfile)
						;
						; build master rr
						ecwi_slice_rr,rcfg,ppar
					endif
					;
					; read in master rr
					...
					ecwi_print_info,ppar,pre,'Input slice rr size, wavelength range', $
						mrsz[1],mrwave0,mrwave1,format='(a,i7,2f11.3)'
					ecwi_print_info,ppar,pre,'Input object   size, wavelength range', $
					...
					ofil = ecwi_get_imname(ppar,imgnum[i],'_mcuber',/nodir)
					...
					ofil = ecwi_get_imname(ppar,imgnum[i],'_vcuber',/nodir)
					...
					ofil = ecwi_get_imname(ppar,imgnum[i],'_icuber',/nodir)
					...
					if file_test(sfil) then begin
						...
						;
						; write out final intensity image
						ofil = ecwi_get_imname(ppar,imgnum[i],'_scuber',/nodir)
					...
					if file_test(nfil) then begin
						...
						;
						; write out final intensity image
						ofil = ecwi_get_imname(ppar,imgnum[i],'_ocuber',/nodir)
						ecwi_write_image,obj,hdr,ofil,ppar
					endif
					;
					; handle the case when no rr frames were taken
				endif else begin
					ecwi_print_info,ppar,pre,'cannot associate with any master rr: '+ $
