...
;-
pro ecwi_test_std,imno,ps=ps,verbose=verbose,display=display
	;
	; setup
	pre = 'ECWI_TEST_STD'
	...
	ecwi_print_info,ppar,pre,'testing inverse sensitivity curve for '+sname
	;
	; get size
	sz = size(icub,/dim)
	;
	...
	;
	; log results
	ecwi_print_info,ppar,pre,'Invsens. Pars: X0, X1, Y0, Y1, Wav0, Wav1', $
	...
	sfw = sfw[sroi]
	fwhm = max(sfw)
	ecwi_print_info,ppar,pre,'reference spectrum FWHM used',fwhm, $
		format='(a,f5.1)'
	;
	...
	ecwi_legend,['Cal. Flux','Obs. Flux','Smoothed'],linesty=[0,0,0], $
	...
	ecwi_legend,['<Resid> = '+strtrim(string(mo[0],format='(g13.3)'),2) + $
	...
	if file_test(eafil) then begin
		...
		ecwi_print_info,ppar,pre,'EA file not found',eafil,/warning
	;
